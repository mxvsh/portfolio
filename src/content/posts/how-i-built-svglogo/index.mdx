...
The implementation is straightforward: serialize the logo config to a JSON object, store it in Cloudflare KV, and return a short ID. The share URL is just `/?s=[id]` — a query param on the homepage. When someone opens it, the config is fetched from KV and loaded into the editor.

KV was the obvious choice once we were on Cloudflare infra — no separate Redis instance to manage, globally distributed, and the pricing is generous enough that a logo tool doesn't have to think about it.
...
After launch, I migrated to TanStack Start — the framework I'd originally tried and dropped on night one in favor of shipping fast with Next.js. A few weeks later, with the product stable and the feedback loop running, it was the right time to revisit it.

Once the pressure of "get it live" was off, the tradeoffs became clearer. TanStack Start's routing model fits the app better, and the performance characteristics on Cloudflare are noticeably better.
